...
- [x] Wire the live model call in `/api/seeds/parse` when `ANTHROPIC_API_KEY` is set: call Claude with ONLY the wish text, validate against `SeedDraft`, fall back to `parseSeedMock` on any error. _(Cycle 34 — fetch-based, no SDK, env-gated + dormant here; validator unit-tested)_
