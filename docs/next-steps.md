...
- [ ] **Per-category pack art**: each pack currently renders one signature scene; give each pack a scene per category (body/creation/…), and bring real (downloaded) library assets into the pack interface. Then decide the home card layout (featured vs stack).
